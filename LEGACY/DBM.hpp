#ifndef DBM_HPP_INCLUDED
#define DBM_HPP_INCLUDED
...
void CCNC(){//Checks for corrupted neural network configs, and rebuild the files
    CSV NNDT;
    std::cout<<"Corruption checker"<<std::endl;
    for(int i=0;i<PVSNC.size();i++){

        NNDT.LCSV(PVSNC[i]);
        //std::cout<<NNDT.DATA[0][0]<<std::endl;
        for(int c=0;c<NNDT.DATA.size();c++){
            for(int f=0;f<NNDT.DATA[c].size();f++){
                if(!(NNDT.DATA[c][f]==NNDT.DATA[c][f])){
                    std::cout<<"Corrupted neural config file"<<std::endl;
                                NNDT.LCSV(PBSNC[i]);
            NNDT.SCSV(PVSNC[i]);
...
class FDBM{///Financial external sources database manager class
public:
...
class DNC {//Data normalizer and compositor for financial use in pairs
public:

    CSV PR;//Data structure containing data to predict
    CSV VR;//Data structure containing data to predict from
    
    FDBM DTMN;//Financial data base manager
...
//Sets the training vector to the require data size
    ...
    bool CompD(){//Ensures and compatilize the data in function of the date, dSs is the size of the data set,0 is for the máximun posible
        ...
        std::vector<double> PFdL;//Vector con estacio en posix time entre fechas
        PFdL.clear();
        //std::cout<<"DBG 2: "<<VR.DATA.size()<<std::endl;
        for(int i=1;i<VR.DATA.size();i++){
          //  std::cout<<"DBG 3: "<<i<<std::endl;
            PFdL.push_back(VR.DATA[i-1][0]-VR.DATA[i][0]);
        }
        double PFMD=MEAN(PFdL);
        ...
        TPdL.clear();
        for(int i=1;i<PR.DATA.size();i++){
            TPdL.push_back(PR.DATA[i-1][0]-PR.DATA[i][0]);
        }
       ...
       if(PR.DATA.size()<=1||VR.DATA.size()<=1){//quandl server has a error
           std::cout<<"Quandl ERR. SRV_KEY:"<<randomIndex<<std::endl;
